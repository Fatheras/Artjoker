...
//const mongoose = require('mongoose'); poka ne nado
...
app.post('/submitComment', function (req, res) {
    ...
    MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection('CSVcollection').insertOne(data, function (err, res) {
            if (err) throw err;
...
app.get('/getComments', (req, res) => {
    MongoClient.connect(url, function (err, db) {
        if (err) throw err;
        var dbo = db.db("CSV");
        dbo.collection("CSVcollection").find({}).toArray(function (err, result) {
            if (err) throw err;
            console.log(result);
            db.close();
